...
def prepare_models_and_tokenizer(args: CustomArguments) -> Tuple[BertForMaskedLM, BertForMaskedLM, BertTokenizer]:
    ...
    student.bert.requires_grad_(False if args.freeze_student_encoder else True)
...
class Runner(pl.LightningModule):
    def __init__(
        ...
        self.christian_ids = christian_ids
        self.jewish_ids = jewish_ids
        self.muslim_ids = muslim_ids
    ...
    def _get_divided_logits(
        ...
        student_christian_logits = student_stereo_logits[
            torch.arange(torch.Tensor.size(batch["stereo_input_ids"])[0]), batch["mask_pos"]
        ][:, self.christian_ids]
        student_jewish_logits = student_stereo_logits[
            torch.arange(torch.Tensor.size(batch["stereo_input_ids"])[0]), batch["mask_pos"]
        ][:, self.jewish_ids]
        student_muslim_logits = student_stereo_logits[
            torch.arange(torch.Tensor.size(batch["stereo_input_ids"])[0]), batch["mask_pos"]
        ][:, self.muslim_ids]
        ...
        return (
            ...
            student_christian_logits,
            student_jewish_logits,
            student_muslim_logits,
    ...
    def _get_bias_loss(
        self,
        student_christian_logits: torch.Tensor,
        student_jewish_logits: torch.Tensor,
        student_muslim_logits: torch.Tensor,
    ) -> torch.Tensor:
        js_div_pq = self.js_div.forward(logits_p=student_christian_logits, logits_q=student_jewish_logits)
        js_div_qr = self.js_div.forward(logits_p=student_jewish_logits, logits_q=student_muslim_logits)
        js_div_rp = self.js_div.forward(logits_p=student_muslim_logits, logits_q=student_christian_logits)
    ...
    def training_step(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        bias_loss = self._get_bias_loss(
            student_christian_logits=student_christian_logits,
            student_jewish_logits=student_jewish_logits,
            student_muslim_logits=student_muslim_logits,
        )
        ...
        self.log(name="religion-mlm-loss", value=loss)
